...
# 引入策略類別
from strategies.TMAC import TMACStrategy, TMACParams

MA_TYPE = "SMA"
STRATEGY = f"TMAC_{MA_TYPE}_DDD"
...
SHORT_PERIODS = [5, 10]
MID_PERIODS = [20, 30, 45]
LONG_PERIODS = [60, 90, 120]

def main():
    for stock in tqdm(STOCK, desc="總股票進度", ncols=100):
        for short_period in tqdm(SHORT_PERIODS, desc="短期進度", ncols=100):
            for mid_period in tqdm(MID_PERIODS, desc="中期進度", ncols=100):
                for long_period in tqdm(LONG_PERIODS, desc="長期進度", ncols=100):
                    if short_period >= mid_period or mid_period >= long_period:
                        continue

                    tqdm.write(f"回測 {STRATEGY} 股票:{stock}, 短期{short_period} 中期{mid_period} 長期{long_period}")
                    
                    # 設定輸出路徑
                    mode = f"1D{str(short_period)}D{str(mid_period)}D{str(long_period)}D"
                    ...
                    # 設定策略參數
                    strategy_params = TMACParams(
                        ma_type=MA_TYPE,
                        short_period=short_period,
                        mid_period=mid_period,
                        long_period=long_period
                    )
                    strategy = TMACStrategy(strategy_params)
